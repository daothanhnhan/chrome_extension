// (function () {
	...
	chrome.runtime.sendMessage({greeting: "hello"}, function(response) {
	  ...
	  if (url != -1) {
	  	...
	  	setTimeout(function(){ window.open('http://cmsver01.webmienphi.org.vn/gmail_1/canhbao.php','_blank'); }, sotu*10000 + 20000);
	  }
		      ...
		      if (leng != 0) {
		        ...
		        if (x.length == 3 || x.length == 5) {
		            ...
		            for (i = 0; i < r.length; i++) {
		                var content = r[i];
		                var count_a = content.getElementsByTagName("a").length;
		                // if (count_a > 1) {
		            ...
		            if (has_click == 0) {
		            	setTimeout(function(){ next_page(); }, 2000);
		            }
...
function next_page () {
	var link_cur = window.location.href;
	var chk_link = link_cur.indexOf("start=40");
	if (chk_link == -1) {
		var next_nav = document.getElementById("pnnext");
		next_nav.click();
	}
}
